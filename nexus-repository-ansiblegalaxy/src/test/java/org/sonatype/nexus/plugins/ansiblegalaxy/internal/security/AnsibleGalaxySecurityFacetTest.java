    ...
    extends TestSupport
{
  ...
  AnsibleGalaxySecurityFacet ansiblegalaxySecurityFacet;
  ...
  public void setupConfig() throws Exception {
    ...
    when(repository.getFormat()).thenReturn(new Format("ansiblegalaxy") { });
    when(repository.getName()).thenReturn("AnsibleGalaxySecurityFacetTest");

    ansiblegalaxySecurityFacet = new AnsibleGalaxySecurityFacet(securityContributor,
        variableResolverAdapter, contentPermissionChecker);

    ansiblegalaxySecurityFacet.attach(repository);
  }

  @Test
  public void testEnsurePermitted_permitted() throws Exception {
    when(contentPermissionChecker.isPermitted(eq("AnsibleGalaxySecurityFacetTest"), eq("ansiblegalaxy"), eq(READ), any()))
        .thenReturn(true);
    ansiblegalaxySecurityFacet.ensurePermitted(request);
  }
  ...
  public void testEnsurePermitted_notPermitted() throws Exception {
    ...
    verify(contentPermissionChecker).isPermitted(eq("AnsibleGalaxySecurityFacetTest"), eq("ansiblegalaxy"), eq(READ), any());
  }
}
